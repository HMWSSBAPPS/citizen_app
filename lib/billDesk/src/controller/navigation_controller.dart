...
import 'package:hmwssb/billDesk/src/controller/sdk_webview_controller.dart';
...
import 'package:hmwssb/billDesk/src/utilities/order_config_validator.dart';
...
class NavigationController extends GetxController {
  ...
  Future<void> _updateParams(
    ...
    if (sdkConfig.isUATEnv == false && Platform.isAndroid) {
  ...
  void _loadWebPage(InAppWebViewController controller, Uri? uri) async {
    if (uri.toString().contains("billdesksdk://web-flow")) {
      ...
      if (sdkConfig.isUATEnv == false && Platform.isIOS) {
        controller.evaluateJavascript(source: """
      setTimeout(()=>{
          updateUrlByKey(${sdkConfig.shouldUseOldUat})
      },500)
